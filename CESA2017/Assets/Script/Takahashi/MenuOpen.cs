...
public class MenuOpen : MonoBehaviour {
	...
	void Update () {
        if (bOpen)
        {
            if (MenuImage.rectTransform.localScale.y < 0.95f)
            {
                MenuImage.rectTransform.localScale += new Vector3(0.0f, fZoomSpeed, 0.0f);
                BottonImage.rectTransform.localPosition -= new Vector3(0.0f, fZoomSpeed * 590, 0.0f);
            }
            else
            {
                MenuImage.rectTransform.localScale = new Vector3(1.0f, 0.95f, 1.0f);    
            }
        }
        else
        {
